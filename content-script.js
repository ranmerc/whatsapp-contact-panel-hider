...
const addVisibilityIcon = async () => {
  ...
  const lastIconInSideHeader = await waitForElm(
    `header > div  div:has(>div[aria-label="Meta AI"])`
  );
...
const addPanelIcon = async () => {
  let isClosed = false;
  const showPanelSVG = `<svg xmlns="http://www.w3.org/2000/svg" aria-label="Show left panel" fill="var(--panel-header-icon)" height="30" width="30" viewBox="0 -960 960 960"><path d="m375-240-43-43 198-198-198-198 43-43 241 241-241 241Z"/></svg>`;
  const hidePanelSVG = `<svg xmlns="http://www.w3.org/2000/svg" aria-label="Hide left panel" fill="var(--panel-header-icon)" height="30" width="30" viewBox="0 -960 960 960"><path d="M561-240 320-481l241-241 43 43-198 198 198 198-43 43Z"/></svg>`;
